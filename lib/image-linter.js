...
function replacer(name, value){
    //console.log("HHHHHHHHHHHHHHHHHHHHHHHHH" + name);
    console.log("IIIIIIIIIIIIIIIIIIIIIIIIIIII" +printObject(value));
    return name +'='+value;
}

ImageLinter.prototype.commandsFromInspect = function (configContents) {
    var commands = [];
    configContents = JSON.parse(configContents);
    if (configContents.Config && configContents.Config.Labels) {
        console.log("found some labels");
        if (Object.keys(configContents.Config.Labels).length > 1) {
            console.log("found some labels");
            commands.push({
                name: 'LABEL',
                raw : 'LABEL dummy',
                args: configContents.Config.Labels
            });
        }
    }
    console.log(printObject(commands));
    return commands;
};
...
ImageLinter.prototype.validate = function (contents) {
    ...
    var commands = this.commandsFromInspect(contents);

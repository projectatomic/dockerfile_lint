 ...
 * @param toFind {key:name}
 * @returns the matched nameVal. If lookup was case insensitive, we also modify "toFind" to the value used to find it
 */
function  nameValMatch(definedNameVals, toFind){
    ...
    for (index in keys){
        key = keys[index];
        if (key === toFind.key){
            return definedNameVals[key];
        }else if (!definedNameVals[key].matchKeyCase && (key.toUpperCase() === (toFind.key.toUpperCase()))){
            toFind.key = key;
            return definedNameVals[key];
        }
    }
...
module.exports.validateNameVals = function (command, context, result) {
    ...
    for (var i = 0; i < names.length; i++) {
        if (definedRules && Object.keys(definedRules).length > 0) {
            ...
            var lookup =  {key:name}; //YUCK!!! but can't pass simple String here due to javascript arg passing
            var ruleMatch = nameValMatch(definedRules,lookup);
            if (ruleMatch) {
                var rule = ruleMatch;
                if (rule.required || rule.required === "recommended") {
                    requiredNameVals[instruction][lookup.key].exists = true; //more YUCK fix later?
                }

...
function isComment(line) {
    console.log("checking for comment: "+ line);
...
function parse(contents, options) {
    ...
    for (i = 0; i < lines.length; i++) {
        if (partialLine && isComment(lines[i].trim())) {
            while (isComment(lines[i].trim())){
              i++;
            }
